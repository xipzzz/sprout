...
1. **Deskew.** `straightenHomeworkFile` (`src/lib/homeworkScan.ts`) turns the photo into a raster and calls `deskewRaster` (`src/lib/deskew.ts`). Four page corners are required, then a perspective warp only. Column-shift bow flatten is not applied — it slices text lines into vertical strips. If the top and bottom still curl apart (`warpTooSevere`), the scan stops and asks for a flatter retake — OCR does not run, and no shredded preview is saved. Loose pen circles are whitened by `suppressLooseInk` before the preview is saved. If the corners are missing, OCR does not run. The top band of the straightened page is read again so a recoverable first question is not dropped when the full-page read starts mid-sheet.
